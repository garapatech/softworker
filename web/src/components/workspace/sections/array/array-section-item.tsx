    ...
    >
      <div className="mb-3 flex flex-col gap-3 border-b pb-3 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          ...
          <p className={cn('mt-1 text-[0.72rem]', itemErrorCount > 0 ? 'text-destructive' : 'text-muted-foreground')}>
            Item {index + 1}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 md:justify-end">
          <button
            type="button"
            className="cursor-pointer rounded-full border border-border bg-muted/40 px-3.5 py-1.5 text-sm text-foreground transition-colors hover:bg-muted/70"
      ...
      <ResumeFieldList
        ...
        workspace={workspace}
        autoFocusFirstField={shouldFocus}
      />
    </article>
  )
}
