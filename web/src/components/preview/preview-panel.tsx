...
export function PreviewPanel({
  ...
  return (
    <Card className="min-h-0 overflow-hidden border-border/80 bg-card xl:sticky xl:top-4 xl:grid xl:h-[calc(100vh-2rem)] xl:grid-rows-[auto_minmax(0,1fr)]">
      <CardHeader className="gap-4 border-b border-border/70 bg-card p-4 sm:p-6 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
          <Button
            variant="outline"
            size="sm"
            className="h-10 w-full rounded-xl border-border/80 bg-card px-3.5 text-sm font-medium text-muted-foreground shadow-none hover:border-border hover:bg-muted/60 hover:text-foreground sm:w-auto"
          ...
          <Button
            variant="default"
            size="sm"
            className="h-10 w-full rounded-xl px-4 text-sm font-semibold shadow-none sm:w-auto"
            onClick={onPrintPdf}
          >
            <span aria-hidden="true" className="text-base leading-none">
              ↓
      ...
      <CardContent className="h-full overflow-auto bg-muted/30 p-3 sm:p-4">
